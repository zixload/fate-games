"""Seated revolver gesture, second pass: the arm goes to the temple from the side.

The first pass only moved the wrist with a two-bone IK: the elbow ended in front
of the face, the hand stayed open and the gun sat beside the wrist. This pass
keeps the same timing (so the server code does not change) and adds:

  * an IK pole that swings the elbow out to the right side,
  * a hand orientation at the temple: fingers toward the head, thumb up,
  * a closed grip from the moment the hand reaches the gun until it lets go.

The exact placement of the Nagant in the hand is tuned in game (/prise), since
the socket frame after the FBX round trip is only visible there.

Run with Steam Blender:
  blender.exe -b --python create_revolver_take_v2.py
"""
...
wrist_temple = temple + Vector((-0.16, 0.03, -0.07))
...
for frame in range(1, FRAMES + 1):
    ...
    if w > 0:
        ...
        wanted = basis(Vector((1, 0.15, 0)).normalized(), Vector((0, 0, 1)))
