...
return function(H, Stubs)
    ...
    H.describe("liars_bar/deck", function()
        ...
        H.it("melange de facon reproductible a rng identique", function()
            local Deck = make_deck(config)
            local a = Deck.Shuffle(Deck.Build(), rng_scripte({ 4, 11, 2, 19, 6 }))
            local b = Deck.Shuffle(Deck.Build(), rng_scripte({ 4, 11, 2, 19, 6 }))
...
Run: `lua tests/run.lua`
Expected: les 74 tests existants toujours verts, plus 8 nouveaux sous `liars_bar/deck`, 0 échec.
