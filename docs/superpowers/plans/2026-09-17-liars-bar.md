...
- Consumes: rien.
- Produces :
  - `data/config.lua` rend la table `{ hand_size = 5, max_play = 3, min_players = 3, max_seats = 4, chambers = 6, shoot_timeout = 15.0 }`
...
return {
    ...
    -- Quatre places, pas six : vingt cartes et des mains de cinq ne servent que
    -- quatre joueurs, cinq en demanderaient vingt-cinq. C'est la table canonique
    -- du jeu et la seule compatible avec l'arithmetique de data/deck.lua.
    min_players = 3,
    max_seats   = 4,
...
- `[INFO][liars] table posee : 4 places`
- cinq lignes `[DEBUG][interactables] enregistre` — quatre chaises et le revolver
